"""server/highresnowc.py

Flask route to handle /highresnowc calls.
"""
...
class HighresnowcQuerySchema(Schema):
...
class HighresnowcOptionSchema(Schema):
    class Meta:
        unknown = EXCLUDE
...
@TILE_API.route('/highresnowc/<path:keys>/<int:tile_z>/<int:tile_x>/<int:tile_y>.png',
                methods=['GET'])
def get_highresnowc(tile_z: int, tile_y: int, tile_x: int, keys: str) -> Response:
    """Return single-band PNG image of requested tile
    ---
    get:
        summary: /highresnowc (tile)
        description: Return single-band PNG image of requested XYZ tile
        parameters:
            - in: path
              schema: HighresnowcQuerySchema
            - in: query
              schema: HighresnowcOptionSchema
        responses:
            200:
                description:
    ...
    return _get_highresnowc_image(keys, tile_xyz)


class HighresnowcPreviewSchema(Schema):
    keys = fields.String(required=True, description='Keys identifying dataset, in order')


@TILE_API.route('/highresnowc/<path:keys>/preview.png', methods=['GET'])
def get_highresnowc_preview(keys: str) -> Response:
    """Return single-band PNG preview image of requested dataset
    ---
    get:
        summary: /highresnowc (preview)
        description: Return single-band PNG preview image of requested dataset
        parameters:
            - in: path
              schema: HighresnowcPreviewSchema
            - in: query
              schema: HighresnowcOptionSchema
        responses:
            200:
            ...
            404:
                description:
                    No dataset found for given key combination
    """
    return _get_highresnowc_image(keys)


def _get_highresnowc_image(keys: str, tile_xyz: Tuple[int, int, int] = None) -> Response:
    from terracotta.handlers.highresnowc import highresnowc

    parsed_keys = [key for key in keys.split('/') if key]

    option_schema = HighresnowcOptionSchema()
    options = option_schema.load(request.args)

    if options.get('colormap', '') == 'explicit':
        options['colormap'] = options.pop('explicit_color_map')

    image = highresnowc(parsed_keys, tile_xyz=tile_xyz, **options)
